...
$ T = t_(11) ketbra(u_1) + sum_(j=2)^n t_(1j) ketbra(u_1, v_j) + sum_(i=2)^n t_(i 1) ketbra(v_i, u_1) + underbrace(sum_(i=2)^n sum_(j=2)^n t_(i j) ketbra(v_i, v_j), T_perp) $ <הגדרתי>
...
$ mel(u_1, T^+T, u_1) = lr((bra(u_1) T^+) (T ket(u_1))) = braket(T u_1) = braket(lambda_1 u_1) = lambda_1 lambda_1^* braket(u_1) = abs(lambda_1)^2 $ <עע1>
...
#שאלה(כותרת: "קומוטטורים", [
  ...
  ואין ניוון עבור הערכים העצמיים של $A$, אז הבסיס העצמי של $A$ הוא הבסיס העצמי של $B$.
...
$ A ket(v) =& A (alpha ket(a\, 1) + beta ket(a\, 2)) = alpha A ket(a\, 1) + beta A ket(a\, 2) \ =& alpha a ket(a\, 1) + beta a ket(a\, 2) = a (alpha ket(a\, 1) + beta ket(a\, 2)) $
