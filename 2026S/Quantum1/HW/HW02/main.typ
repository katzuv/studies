...
#סעיף[
  הראו כי הערכים העצמיים של אופרטור אוניטרי מקיימים שערכם המוחלט הוא $1$.
]
ב@עע1 ראינו כי מתקיים:
$ mel(u_1, T^+T, u_1) = abs(lambda_1)^2 $
משום שכעת $T$ אוניטרי, נקבל, נזכור כי בחרנו $ket(u_1)$ בגודל 1, לכן:
$ mel(u_1, T^+T, u_1) = mel(u_1, I, u_1) = 1 $
מכאן נקבל:
#תשובה[
  $ abs(lambda_1)^2 = 1 => abs(lambda_1) = 1 $
  בתהליך האינדוקציה שפירטנו בסעיף א'
]