...
def analyze_and_plot_fh_files(file_paths, output_svg="fh_characteristic_curves.svg"):
    ...
    for path in file_paths:
        ...
        try:
            ...
            heater_match = re.search(r"Cathode Heater Current \(mA\) =\s*([\d.]+)", metadata_text)
            ...
            if has_multiple_peaks:
                ...
                excitation_energy, std_error_diff, contact_potential, contact_pot_error = np.nan, np.nan, np.nan, np.nan
...
def analyze_ionization_experiment(ionization_file_path, contact_potential_V, contact_pot_error_V=0.0, output_svg="fh_ionization_curve.svg"):
    """
    ...
    ax.errorbar(raw_voltage, raw_current, xerr=v_error_inst, yerr=i_error_inst, 
                fmt='.', color='#1f77b4', label='Ion Current Data ($I$)',
                linewidth=0.5, elinewidth=0.3, alpha=0.5, capsize=1, zorder=2)
    
    ax.axhline(y=statistical_noise_ceiling, color='orange', linestyle=':', label=r'5$\sigma$ Noise Floor Ceiling')
    ...
    results_box_text = (
        ...
        rf"True $E_{{ion}}$: {true_ionization_energy:.3f} $\pm$ {total_error:.3f} eV\n"
...
if __name__ == "__main__":
    ...
    if pd.isna(c_pot): c_pot = 1.112 # fallback
    if pd.isna(c_pot_err): c_pot_err = 0.06 # fallback
