...
for i in range(6):
    theoretical_1_err[i] = propagate_error(
        get_freq_free,
        np.array((d, F, l, alpha, m, L, N_arr[i])),
...
plt.plot(
    range_1,
    get_freq_free(d, F, l, alpha, m, L, range_1),
    "--",
    label="theoretical graph",
)
...
plt.plot(
    range_2,
    get_freq_held(d, F, l, alpha, m, L, range_2),
    "--",
    label="theoretical graph",
)
plt.ylabel("Frequency [Hz]")
