...
def parse_single_run(file_path: Path):
    ...
    phi_motor_guess = np.arctan2(
        np.sum(motor_ticks * np.cos(2 * np.pi * f * time)),
        np.sum(motor_ticks * np.sin(2 * np.pi * f * time)),
    )

    phi_mass_guess = np.arctan2(
        np.sum(mass_ticks * np.cos(2 * np.pi * f * time)),
    ...
    mass_parameters, popt_ma = scipy.optimize.curve_fit(
        simple.sine_func, time, mass_ticks, p0=p0_ma, sigma=ticks_err
    )
    motor_parameters, popt_mo = scipy.optimize.curve_fit(
        simple.sine_func, time, motor_ticks, p0=p0_mo, sigma=ticks_err
    )
    ...
    amplitude_ratio_err = np.sqrt(
        mass_parameters_err[0] ** 2 / motor_parameters[0] ** 2
        + (mass_parameters[0] ** 2)
...
plt.errorbar(
    frequencies,
    amplitude_ratios,
    xerr=frequencies_errs,
    yerr=amplitude_ratios_err,
    fmt=".",
    label="results",
)
p0_amp = np.array(
    [
        driven_constants.k1 / driven_constants.CART_MASS,
        driven_constants.NATURAL_FREQUENCY,
        2.72074,
    ]
)
amp_fit, popt_amp = scipy.optimize.curve_fit(
    amplitude_ratio_model_cfit,
    frequencies,
    amplitude_ratios,
    sigma=amplitude_ratios_err,
    p0=p0_amp,
)
...
plt.plot(
    lin,
    amplitude_ratio_model_cfit(lin, amp_fit[0], amp_fit[1], amp_fit[2]),
    "--",
    label="fit",
)
plt.grid()
...
def phase_diff_model(motor_frequency, natural_frequency, tau):
    return (
        np.arctan2(motor_frequency / tau, natural_frequency**2 - motor_frequency**2)
        - np.pi
    )
...
fit_diff, popt_diff = scipy.optimize.curve_fit(
    phase_diff_model, frequencies, diffs, sigma=diffs_err, bounds=bounds_diff
)
fit_diff_err = np.sqrt(np.diag(popt_diff))
plt.errorbar(
    frequencies, diffs, xerr=frequencies_errs, yerr=diffs_err, fmt=".", label="results"
)
lin = np.linspace(0, driven_constants.NATURAL_FREQUENCY * 2, 2000)
phase_diff = [
    phase_diff_model(freq, driven_constants.NATURAL_FREQUENCY, 2.72074) for freq in lin
]
# plt.plot(lin, phase_diff, label = "theoretical graph")
plt.plot(
    lin,
    phase_diff_model(lin, driven_constants.NATURAL_FREQUENCY, fit_diff[1]),
    "--",
    label="fit",
)
plt.grid()
