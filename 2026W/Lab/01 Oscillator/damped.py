...
def process_run(path: Path):
    ...
    popt, pcov = scipy.optimize.curve_fit(
        damped_model, time, ticks, p0=p0, sigma=ticks_err
    )
    ...
    plt.plot(
        t_fit,
        amplitude_model(t_fit, -amplitude_fit, tau_fit) * 100,
        "g-",
        label="Amplitude fit",
        zorder=0,
    )
    ...
    theoretical_omega = np.sqrt(
        constants.SPRING_CONSTANT / constants.CART_MASS - (1 / (4 * tau_fit**2))
    )
    ...
    sigma_A = np.sqrt(
        (dA_dK * sigma_K) ** 2 + (dA_dm * sigma_m) ** 2 + (dA_dtau * sigma_tau) ** 2
    )
    theoretical_omega_error = 0.5 / np.sqrt(A) * sigma_A
    error = (np.abs(frequency_fit - theoretical_omega) / theoretical_omega) * 100

    print(
        f"Theory: {theoretical_omega}±{theoretical_omega_error} | Error percentage of frequency from theory: {error:.2f}%"
    )
