...
speed_in_air_err = propagate_error(
    calc_speed,
    (
        CYLINDER_LENGTH,
        time_in_air,
...
speed_in_helium_err = propagate_error(
    calc_speed,
    (
        CYLINDER_LENGTH,
        time_in_helium,
...
speed_in_co2_err = propagate_error(
    calc_speed, (CYLINDER_LENGTH, time_in_co2), (0.01, 0.002e-3)
)
...
print("-" * len(header))
print(
    f"{'Air':<8}{time_in_air * 1000:12.3f}{speed_in_air:14.2f}{speed_in_air_err:14.2f}"
)
print(
    f"{'He':<8}{time_in_helium * 1000:12.3f}{speed_in_helium:14.2f}{speed_in_helium_err:14.2f}"
)
print(
    f"{'CO2':<8}{time_in_co2 * 1000:12.3f}{speed_in_co2:14.2f}{speed_in_co2_err:14.2f}"
)
...
theo_air_err = propagate_error(
    theo_speed,
    (gama_air, const, temp_1, molar_mass_air),
    (gama_air_err, const_err, temp_err, molar_mass_air_err),
)
theo_helium_err = propagate_error(
    theo_speed,
    (gama_helium, const, temp_2, molar_mass_helium),
    (gama_helium_err, const_err, temp_err, molar_mass_helium_err),
)
theo_co2_err = propagate_error(
    theo_speed,
    (gama_co2, const, temp_1, molar_mass_co2),
    (gama_co2_err, const_err, temp_err, molar_mass_co2_err),
)

print(
    f"the theoretical speed of sound in air is {theo_air:.2f} ± {theo_air_err:.2f} [m/s]"
)
print(
    f"the theoretical speed of sound in Helium is {theo_helium:.2f} ± {theo_helium_err:.2f} [m/s]"
)
print(
    f"the theoretical speed of sound in CO2 is {theo_co2:.2f} ± {theo_co2_err:.2f} [m/s]"
)
