...
class PDFMerger:
    ...
    def merge_pdfs(
        self, pdf_files: list[Path], output_path: Path, toc_titles: list[str] = None
    ) -> Path:
        """
    ...
    def _add_toc_page(self):
        ...
        toc_page.insert_text(
            (50, 50), "Table of Contents", fontsize=20, fontname="helv", color=(0, 0, 0)
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python pdf_merger.py <lectures_folder> <tirgul_folder> <output_folder> [course_name]"
        )
        sys.exit(1)
