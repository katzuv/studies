...
class DriveHandler:
    ...
    def download_file(self, file_id: str, destination: Path) -> Path:
        ...
        with io.FileIO(str(destination), "wb") as fh:
            ...
            with tqdm(
                total=100, unit="%", desc=f"Downloading {destination.name}"
            ) as pbar:
                while not done:
    ...
    def download_folder_pdfs(
        self, folder_id: str, destination_folder: Path
    ) -> list[Path]:
        """
    ...
    def upload_file(
        self, file_path: Path, folder_id: str = None, file_name: str = None
    ) -> str:
        """
...
def download_course_files(
    drive: DriveHandler, course_name: str, course_folder_id: str, local_base: Path
) -> dict[str, list[Path]]:
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python drive_handler.py <credentials_path> <folder_id> [output_dir]"
        )
        sys.exit(1)
